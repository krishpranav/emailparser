...
fn decode_word(encoded: &str) -> Option<String> {
    ...
    let (cow, _) = charset.decode_without_bom_handling(&decode);
    Some(cow.into_owned())
}

// Tokenizes a single line of the header and proces a vector of tokens.
fn tokenize_header_line(line: &str) -> Vec<Headertoken> {
    ...
    loop {
        match find_from(line, ix_search, "=?") {
            Some(v) => {
                let ix_begin = v + 2;
                if !is_boundary(line, ix_begin.check_sub(3)) {
...
fn tokenize_header(value: &str) -> Vec<HeaderToken> {
    ...
    while let Some(line) = lines.next().map(str::trim_start) {
        ...
        let mut line_tokens = tokenize_header(line);
    ...
    for tok in tokens {
        match &tok {
            ...
            HeaderToken::Newline(_) => {
                ...
                } else if let Some(HeaderToken::DecodeWord(_)) = saved_token {
            ...
            HeaderToken::DecodedWord(_) => {
                saved_token = Some(HeaderToken::DecodeWord(String::new()));
                result.push(tok);
            }
        }
    }
